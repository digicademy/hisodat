...
class PersonsRepository extends CommonRepository
{
    ...
    /**
     * Performs a MySQL Fulltext search on the persons repository using a statement
     *
    ...
    public function findByFulltext($arguments = array(), $settings = array())
    {
        ...
        // set statement
        $statement = 'SELECT tx_hisodat_domain_model_persons.uid FROM tx_hisodat_domain_model_persons';
        ...
        if (!empty($searchwords)) {
            ...
            $indexFields = GeneralUtility::trimExplode(',', $settings['index']['mysql']['fields']['persons'], 1);
            $index = implode(',', $indexFields);

            $statement .= ' WHERE MATCH (' . $index . ') AGAINST (' . $searchwords . ' IN BOOLEAN MODE)';

            // pids
            $statement .= ' AND tx_hisodat_domain_model_persons.pid IN(' . (int)$settings['recordPids']['persons'] . ')';

            // enable fields
            $statement .= ' AND tx_hisodat_domain_model_persons.hidden = 0 AND tx_hisodat_domain_model_persons.deleted = 0';

            // ordering
            $statement .= ' ORDER BY tx_hisodat_domain_model_persons.name ASC';
